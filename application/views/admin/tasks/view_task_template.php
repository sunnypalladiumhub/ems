...
<div class="modal-header task-single-header" data-task-single-id="<?php echo $task->id; ?>" data-status="<?php echo $task->status; ?>">
    <?php if ($this->input->get('opened_from_lead_id')) { ?>
        <a href="#" onclick="init_lead(<?php echo $this->input->get('opened_from_lead_id'); ?>);
                    return false;" class="back-to-from-task color-white" data-placement="left" data-toggle="tooltip" data-title="<?php echo _l('back_to_lead'); ?>">
    ...
    <?php if ($task->is_public == 0) { ?>
        <small class="no-margin color-white">
            ...
            <?php if (has_permission('tasks', '', 'edit')) { ?> -
                <a href="#" class="color-white text-has-action"onclick="make_task_public(<?php echo $task->id; ?>);
                                return false;">
...
<div class="modal-body">
    <div class="row">
        <div class="col-md-8 task-single-col-left">
            <?php
            if (total_rows(db_prefix() . 'taskstimers', array('end_time' => NULL, 'staff_id !=' => get_staff_user_id(), 'task_id' => $task->id)) > 0) {
                                ...
                                <ul>
                                    <?php foreach ($milestones as $milestone) { ?>
                                        <?php if ($task->milestone != $milestone['id']) { ?>
                                            <li>
                                                <a href="#" onclick="task_change_milestone(<?php echo $milestone['id']; ?>,<?php echo $task->id; ?>);
                                                                            return false;">
            ...
            <?php if ($task->status != Tasks_model::STATUS_COMPLETE && ($task->current_user_is_assigned || has_permission('tasks', '', 'edit') || $task->current_user_is_creator)) { ?>
                <p class="no-margin pull-left" style="<?php echo 'margin-' . (is_rtl() ? 'left' : 'right') . ':5px !important'; ?>">
                    <a href="#" class="btn btn-info" id="task-single-mark-complete-btn" autocomplete="off" data-loading-text="<?php echo _l('wait_text'); ?>" onclick="mark_complete(<?php echo $task->id; ?>);
                                return false;" data-toggle="tooltip" title="<?php echo _l('task_single_mark_as_complete'); ?>">
            ...
            <?php } else if ($task->status == Tasks_model::STATUS_COMPLETE && ($task->current_user_is_assigned || has_permission('tasks', '', 'edit') || $task->current_user_is_creator)) { ?>
                <p class="no-margin pull-left" style="<?php echo 'margin-' . (is_rtl() ? 'left' : 'right') . ':5px !important'; ?>">
                    <a href="#" class="btn btn-default" id="task-single-unmark-complete-btn" autocomplete="off" data-loading-text="<?php echo _l('wait_text'); ?>" onclick="unmark_complete(<?php echo $task->id; ?>);
                                return false;" data-toggle="tooltip" title="<?php echo _l('task_unmark_as_complete'); ?>">
                        <i class="fa fa-check"></i>
                    </a>
                </p>
            <?php } ?>
            <?php if (has_permission('tasks', '', 'create') && count($task->timesheets) > 0) { ?>
                <p class="no-margin pull-left mright5">
                    <a href="#" class="btn btn-default mright5" data-toggle="tooltip" data-title="<?php echo _l('task_statistics'); ?>" onclick="task_tracking_stats(<?php echo $task->id; ?>);
                                return false;">
            ...
            <?php } ?>
            <p class="no-margin pull-left mright5">
                <a href="#" class="btn btn-default mright5" data-toggle="tooltip" data-title="<?php echo _l('task_timesheets'); ?>"onclick="slideToggle('#task_single_timesheets');
                        return false;">
            ...
            <div class="clearfix"></div>
            <hr class="hr-10" />
            <div id="task_single_timesheets" class="<?php
            if (!$this->session->flashdata('task_single_timesheets_open')) {
                echo 'hide';
            }
            ?>">
            ...
            <?php if (has_permission('tasks', '', 'edit')) { ?><a href="#" onclick="edit_task_inline_description(this,<?php echo $task->id; ?>);
                        return false;" class="pull-left mtop10 mleft5 font-medium-xs"><i class="fa fa-pencil-square-o"></i></a>
               <?php } ?>
            <div class="clearfix"></div>
            <?php
            ...
            <div class="clearfix"></div>
            <hr />
            <a href="#" onclick="add_task_checklist_item('<?php echo $task->id; ?>');
                    return false" class="mbot10 inline-block">
                <span class="new-checklist-item"><i class="fa fa-plus-circle"></i>
                    <?php echo _l('add_checklist_item'); ?>
                </span>
            </a>
            <div class="form-group no-mbot checklist-templates-wrapper simple-bootstrap-select task-single-checklist-templates<?php
                ...
                <div class="row task_attachments_wrapper">
                    <div class="col-md-12" id="attachments">
                        ...
                        <div class="row">
                            ...
                            $rootcause_attachments = array();
                            $corrective_attachments = array();
                            $preventive_attachments = array();
                            $comments_attachments = array();
                            $attachments_data = array();
                            $show_more_link_task_attachments = hooks()->apply_filters('show_more_link_task_attachments', 2);
                            foreach ($task->attachments as $allattachment) {
                                foreach ($allattachment as $attachment) {
                                    ?>
                                    <?php ob_start(); ?>
                                    <div data-num="<?php echo $i; ?>" data-commentid="<?php echo $attachment['comment_file_id']; ?>" data-comment-attachment="<?php echo $attachment['task_comment_id']; ?>" data-task-attachment-id="<?php echo $attachment['id']; ?>" class="task-attachment-col col-md-6<?php
                                    if ($i > $show_more_link_task_attachments) {
                                            ...
                                            ?>">
                                                <div class="mbot10 pull-right task-attachment-user">
                                                    <?php if ($attachment['staffid'] == get_staff_user_id() || is_admin()) { ?>
                                                        <a href="#" class="pull-right" onclick="remove_task_attachment(this,<?php echo $attachment['id']; ?>);
                                                                                return false;">
                                                    ...
                                                    } else if ((!empty($attachment['thumbnail_link']) || !empty($attachment['external'])) && !empty($attachment['thumbnail_link'])) {
                                                        $is_image = true;
                                                ...
                                                ?>">
                                                         ...
                                                         if (!$isHtml5Video) {
                                                             ?>
                                                        <a href="<?php echo (!$externalPreview ? $href_url : $externalPreview); ?>" target="_blank"<?php if ($is_image) { ?> data-lightbox="task-attachment"<?php } ?> class="<?php
                                                        if ($isHtml5Video) {
                                                            echo 'video-preview';
                                                        }
                                                        ?>">
                                    ...
                                    $attachments_dataAll[$attachment['rel_type']][$attachment['id']] = ob_get_contents();
                                    if ($attachment['task_comment_id'] != 0) {
                                        if (isset($attachment['rel_type']) && $attachment['rel_type'] == "task-root_cause") {
                                            $rootcause_attachments[$attachment['task_comment_id']][$attachment['id']] = $attachments_data[$attachment['id']];
                                        } else if (isset($attachment['rel_type']) && $attachment['rel_type'] == "task-corrective") {
                                            $corrective_attachments[$attachment['task_comment_id']][$attachment['id']] = $attachments_data[$attachment['id']];
                                        } else if (isset($attachment['rel_type']) && $attachment['rel_type'] == "task-preventive") {
                                            $preventive_attachments[$attachment['task_comment_id']][$attachment['id']] = $attachments_data[$attachment['id']];
                                        } else {
                                            $comments_attachments[$attachment['task_comment_id']][$attachment['id']] = $attachments_data[$attachment['id']];
                                        }
                                    }
                                    ob_end_clean();
                                    echo $attachments_dataAll[$attachment['rel_type']][$attachment['id']];
                                    ?>
                                    <?php
                                    $i++;
                                }
                    ...
                    <?php if (($i - 1) > $show_more_link_task_attachments) { ?>
                        <div class="col-md-12" id="show-more-less-task-attachments-col">
                            <a href="#" class="task-attachments-more" onclick="slideToggle('.task_attachments_wrapper .task-attachment-col-more', task_attachments_toggle);
                                            return false;"><?php echo _l('show_more'); ?></a>
                            <a href="#" class="task-attachments-less hide" onclick="slideToggle('.task_attachments_wrapper .task-attachment-col-more', task_attachments_toggle);
                                            return false;"><?php echo _l('show_less'); ?></a>
                        </div>
                    <?php } ?>
                    <div class="col-md-12 text-center">
                        <hr />
                        <a href="<?php echo admin_url('tasks/download_files_all/0/' . $task->id); ?>" class="bold">
            ...
            <?php } ?>
            <hr />
            <a href="#" id="taskCommentSlide" onclick="slideToggle('.tasks-comments');
                    return false;">
                <h4 class="mbot20 font-medium"><?php echo _l('task_comments'); ?></h4>
            </a>
            <div class="tasks-comments inline-block full-width simple-editor"<?php
                ...
                <div id="task-comments" class="mtop10">
                  ...
                  <button type="button" class="btn btn-default pull-right mright5" onclick="cancel_edit_comment(' . $comment['id'] . ')">' . _l('cancel') . '</button>
                        ...
                        if ($comment['file_id'] != 0) {
                            $comment['content'] = str_replace('[task_attachment]', '<div class="clearfix"></div>' . $attachments_dataAll['task'][$comment['file_id']], $comment['content']);
            ...
            <!--Root cause section-->
            <a href="#" id="" onclick="slideToggle('.tasks-root-cause');
                    return false;">
                <h4 class="mbot20 font-medium"><?php echo _l('task_root_cause'); ?></h4>
            </a>
            <div class="tasks-root-cause inline-block full-width simple-editor"<?php
            if (count($task->root_cause) == 0) {
                echo ' style="display:none"';
            }
            ?>>
                     <?php echo form_open_multipart(admin_url('tasks/add_task_root_cause'), array('id' => 'task-root-cause-form', 'class' => 'dropzone dropzone-manual', 'style' => 'min-height:auto;background-color:#fff;')); ?>
                <textarea name="root_cause" placeholder="<?php echo _l('task_single_add_new_root_cause'); ?>" id="task_root_cause" rows="3" class="form-control ays-ignore"></textarea>
                <div id="dropzoneTaskRootCause" class="dropzoneDragArea dz-default dz-message hide task-comment-dropzone">
                    <span><?php echo _l('drop_files_here_to_upload'); ?></span>
                </div>
                <div class="dropzone-task-root-cause-previews dropzone-previews"></div>
                <button type="button" class="btn btn-info mtop10 pull-right hide" id="addTaskRootCauseBtn" autocomplete="off" data-loading-text="<?php echo _l('wait_text'); ?>" onclick="add_task_root_cause('<?php echo $task->id; ?>');" data-root-cause-task-id="<?php echo $task->id; ?>">
                    <?php echo _l('task_single_add_new_root_cause'); ?>
                </button>
                <?php echo form_close(); ?>
                <div class="clearfix"></div>
                <?php
                ...
                <div id="task-comments" class="mtop10">
                    <?php
                    $rootcause = '';
                    $len = count($task->root_cause);
                    $i = 0;

                    foreach ($task->root_cause as $root) {
                        $rootcause .= '<div id="root_cause_' . $root['id'] . '" data-commentid="' . $root['id'] . '" data-task-attachment-id="' . $root['file_id'] . '" class="tc-content task-comment' . (strtotime($root['dateadded']) >= strtotime('-16 hours') ? ' highlight-bg' : '') . '">';
                        $rootcause .= '<a data-task-root-cause-href-id="' . $root['id'] . '" href="' . admin_url('tasks/view/' . $task->id) . '#root_cause_' . $root['id'] . '" class="task-date-as-comment-id"><small><span class="text-has-action inline-block mbot5" data-toggle="tooltip" data-title="' . _dt($root['dateadded']) . '">' . time_ago($root['dateadded']) . '</span></small></a>';
                        if ($root['staffid'] != 0) {
                            $rootcause .= '<a href="' . admin_url('profile/' . $root['staffid']) . '" target="_blank">' . staff_profile_image($root['staffid'], array(
                                        'staff-profile-image-small',
                                        'media-object img-circle pull-left mright10'
                                    )) . '</a>';
                        } elseif ($root['contact_id'] != 0) {
                            $rootcause .= '<img src="' . contact_profile_image_url($root['contact_id']) . '" class="client-profile-image-small media-object img-circle pull-left mright10">';
                        }
                        if ($root['staffid'] == get_staff_user_id() || is_admin()) {
                            $root_added = strtotime($root['dateadded']);
                            $minus_1_hour = strtotime('-1 hours');
                            if (get_option('client_staff_add_edit_delete_task_comments_first_hour') == 0 || (get_option('client_staff_add_edit_delete_task_comments_first_hour') == 1 && $root_added >= $minus_1_hour) || is_admin()) {
                                $rootcause .= '<span class="pull-right"><a href="#" onclick="remove_task_root_cause(' . $root['id'] . '); return false;"><i class="fa fa-times text-danger"></i></span></a>';
                                $rootcause .= '<span class="pull-right mright5"><a href="#" onclick="edit_task_root_cause(' . $root['id'] . '); return false;"><i class="fa fa-pencil-square-o"></i></span></a>';
                            }
                        }
                        $rootcause .= '<div class="media-body">';
                        if ($root['staffid'] != 0) {
                            $rootcause .= '<a href="' . admin_url('profile/' . $root['staffid']) . '" target="_blank">' . $root['staff_full_name'] . '</a> <br />';
                        } elseif ($root['contact_id'] != 0) {
                            $rootcause .= '<span class="label label-info mtop5 mbot5 inline-block">' . _l('is_customer_indicator') . '</span><br /><a href="' . admin_url('clients/client/' . get_user_id_by_contact_id($root['contact_id']) . '?contactid=' . $root['contact_id']) . '" class="pull-left" target="_blank">' . get_contact_full_name($root['contact_id']) . '</a> <br />';
                        }
                        $rootcause .= '<div data-edit-root_cause="' . $root['id'] . '" class="hide edit-task-comment"><textarea rows="5" id="task_root_cause_' . $root['id'] . '" class="ays-ignore form-control">' . str_replace('[task_attachment]', '', $root['content']) . '</textarea>
                  <div class="clearfix mtop20"></div>
                  <button type="button" class="btn btn-info pull-right" onclick="save_edited_root_cause(' . $root['id'] . ',' . $task->id . ')">' . _l('submit') . '</button>
                  <button type="button" class="btn btn-default pull-right mright5" onclick="cancel_edit_root_cause(' . $root['id'] . ')">' . _l('cancel') . '</button>
                  </div>';
                        if ($root['file_id'] != 0) {
                            $root['content'] = str_replace('[task_attachment]', '<div class="clearfix"></div>' . $attachments_dataAll['task-root_cause'][$root['file_id']], $root['content']);
                            // Replace lightbox to prevent loading the image twice
                            $root['content'] = str_replace('data-lightbox="task-attachment"', 'data-lightbox="task-attachment-comment"', $root['content']);
                        } else if (count($root['attachments']) > 0 && isset($rootcause_attachments[$root['id']])) {
                            $root_attachments_html = '';
                            foreach ($rootcause_attachments[$root['id']] as $root_attachment) {
                                $root_attachments_html .= trim($root_attachment);
                            }
                            $root['content'] = str_replace('[task_attachment]', '<div class="clearfix"></div>' . $root_attachments_html, $root['content']);
                            // Replace lightbox to prevent loading the image twice
                            $root['content'] = str_replace('data-lightbox="task-attachment"', 'data-lightbox="task-comment-files"', $root['content']);
                            $root['content'] .='<div class="clearfix"></div>';
                            $root['content'] .='<div class="text-center download-all">
                   <hr class="hr-10" />
                   <a href="' . admin_url('tasks/download_files_all/1/' . $task->id . '/' . $root['id']) . '" class="bold">' . _l('download_all') . ' (.zip)
                   </a>
                   </div>';
                        }
                        $rootcause .= '<div class="comment-content mtop10">' . app_happy_text(check_for_links($root['content'])) . '</div>';
                        $rootcause .= '</div>';
                        if ($i >= 0 && $i != $len - 1) {
                            $rootcause .= '<hr class="task-info-separator" />';
                        }
                        $rootcause .= '</div>';
                        $i++;
                    }
            ...
            <!--End Root cause section-->

            <hr/>

            <!--Corrective action section-->
            <a href="#" id="" onclick="slideToggle('.tasks-corrective');
                    return false;">
                <h4 class="mbot20 font-medium"><?php echo _l('task_corrective_action'); ?></h4>
            </a>
            <div class="tasks-corrective inline-block full-width simple-editor"<?php
            if (count($task->corrective) == 0) {
                echo ' style="display:none"';
            }
            ?>>
                     <?php echo form_open_multipart(admin_url('tasks/add_task_corrective_action'), array('id' => 'task-corrective-action-form', 'class' => 'dropzone dropzone-manual', 'style' => 'min-height:auto;background-color:#fff;')); ?>
                <textarea name="corrective_action" placeholder="<?php echo _l('task_single_add_new_corrective_action'); ?>" id="task_corrective_action" rows="3" class="form-control ays-ignore"></textarea>
                <div id="dropzoneTaskCorrectiveAction" class="dropzoneDragArea dz-default dz-message hide task-comment-dropzone">
                    <span><?php echo _l('drop_files_here_to_upload'); ?></span>
                </div>
                <div class="dropzone-task-corrective-action-previews dropzone-previews"></div>
                <button type="button" class="btn btn-info mtop10 pull-right hide" id="addTaskCorrectiveActionBtn" autocomplete="off" data-loading-text="<?php echo _l('wait_text'); ?>" onclick="add_task_corrective_action('<?php echo $task->id; ?>');" data-corrective-task-id="<?php echo $task->id; ?>">
                    <?php echo _l('task_single_add_new_corrective_action'); ?>
                </button>
                <?php echo form_close(); ?>
                <div class="clearfix"></div>
                <?php
                ...
                <div id="task-comments" class="mtop10">
                    <?php
                    $rootcause = '';
                    $len = count($task->corrective);
                    $i = 0;

                    foreach ($task->corrective as $root) {
                        $rootcause .= '<div id="corrective_' . $root['id'] . '" data-commentid="' . $root['id'] . '" data-task-attachment-id="' . $root['file_id'] . '" class="tc-content task-comment' . (strtotime($root['dateadded']) >= strtotime('-16 hours') ? ' highlight-bg' : '') . '">';
                        $rootcause .= '<a data-task-corrective-href-id="' . $root['id'] . '" href="' . admin_url('tasks/view/' . $task->id) . '#corrective_' . $root['id'] . '" class="task-date-as-comment-id"><small><span class="text-has-action inline-block mbot5" data-toggle="tooltip" data-title="' . _dt($root['dateadded']) . '">' . time_ago($root['dateadded']) . '</span></small></a>';
                        if ($root['staffid'] != 0) {
                            $rootcause .= '<a href="' . admin_url('profile/' . $root['staffid']) . '" target="_blank">' . staff_profile_image($root['staffid'], array(
                                        'staff-profile-image-small',
                                        'media-object img-circle pull-left mright10'
                        ...
                        if ($root['staffid'] == get_staff_user_id() || is_admin()) {
                            ...
                            $minus_1_hour = strtotime('-1 hours');
                            if (get_option('client_staff_add_edit_delete_task_comments_first_hour') == 0 || (get_option('client_staff_add_edit_delete_task_comments_first_hour') == 1 && $root_added >= $minus_1_hour) || is_admin()) {
                                $rootcause .= '<span class="pull-right"><a href="#" onclick="remove_task_corrective_action(' . $root['id'] . '); return false;"><i class="fa fa-times text-danger"></i></span></a>';
                                $rootcause .= '<span class="pull-right mright5"><a href="#" onclick="edit_task_corrective_action(' . $root['id'] . '); return false;"><i class="fa fa-pencil-square-o"></i></span></a>';
                            }
                        }
                        $rootcause .= '<div class="media-body">';
                        if ($root['staffid'] != 0) {
                        ...
                        $rootcause .= '<div data-edit-corrective="' . $root['id'] . '" class="hide edit-task-comment"><textarea rows="5" id="task_corrective_' . $root['id'] . '" class="ays-ignore form-control">' . str_replace('[task_attachment]', '', $root['content']) . '</textarea>
                  <div class="clearfix mtop20"></div>
                  <button type="button" class="btn btn-info pull-right" onclick="save_edited_corrective_action(' . $root['id'] . ',' . $task->id . ')">' . _l('submit') . '</button>
                  <button type="button" class="btn btn-default pull-right mright5" onclick="cancel_edit_corrective_action(' . $root['id'] . ')">' . _l('cancel') . '</button>
                  </div>';
                        if ($root['file_id'] != 0) {
                            $root['content'] = str_replace('[task_attachment]', '<div class="clearfix"></div>' . $attachments_dataAll['task-root_cause'][$root['file_id']], $root['content']);
                            // Replace lightbox to prevent loading the image twice
                            $root['content'] = str_replace('data-lightbox="task-attachment"', 'data-lightbox="task-attachment-comment"', $root['content']);
                        } else if (count($root['attachments']) > 0 && isset($corrective_attachments[$root['id']])) {
                            $root_attachments_html = '';
                            foreach ($corrective_attachments[$root['id']] as $root_attachment) {
                                $root_attachments_html .= trim($root_attachment);
                            }
                            $root['content'] = str_replace('[task_attachment]', '<div class="clearfix"></div>' . $root_attachments_html, $root['content']);
                            // Replace lightbox to prevent loading the image twice
                            ...
                            $root['content'] .='<div class="text-center download-all">
                   <hr class="hr-10" />
                   <a href="' . admin_url('tasks/download_files_all/2/' . $task->id . '/' . $root['id']) . '" class="bold">' . _l('download_all') . ' (.zip)
                   </a>
                   </div>';
                        }
                        $rootcause .= '<div class="comment-content mtop10">' . app_happy_text(check_for_links($root['content'])) . '</div>';
            ...
            <!--End Corrective action section-->

            <hr/>

            <!--Preventive action section-->
            <a href="#" id="" onclick="slideToggle('.tasks-preventive');
                    return false;">
                <h4 class="mbot20 font-medium"><?php echo _l('task_preventive_action'); ?></h4>
            </a>
            <div class="tasks-preventive inline-block full-width simple-editor"<?php
            if (count($task->preventive) == 0) {
                echo ' style="display:none"';
            }
            ?>>
                     <?php echo form_open_multipart(admin_url('tasks/add_task_preventive_action'), array('id' => 'task-preventive-action-form', 'class' => 'dropzone dropzone-manual', 'style' => 'min-height:auto;background-color:#fff;')); ?>
                <textarea name="preventive_action" placeholder="<?php echo _l('task_single_add_new_preventive_action'); ?>" id="task_preventive_action" rows="3" class="form-control ays-ignore"></textarea>
                <div id="dropzoneTaskPreventiveAction" class="dropzoneDragArea dz-default dz-message hide task-comment-dropzone">
                    <span><?php echo _l('drop_files_here_to_upload'); ?></span>
                </div>
                <div class="dropzone-task-preventive-action-previews dropzone-previews"></div>
                <button type="button" class="btn btn-info mtop10 pull-right hide" id="addTaskPreventiveActionBtn" autocomplete="off" data-loading-text="<?php echo _l('wait_text'); ?>" onclick="add_task_preventive_action('<?php echo $task->id; ?>');" data-preventive-task-id="<?php echo $task->id; ?>">
                    <?php echo _l('task_single_add_new_preventive_action'); ?>
                </button>
                <?php echo form_close(); ?>
                <div class="clearfix"></div>
                <?php
                ...
                <div id="task-comments" class="mtop10">
                    <?php
                    $rootcause = '';
                    $len = count($task->preventive);
                    $i = 0;

                    foreach ($task->preventive as $root) {
                        $rootcause .= '<div id="corrective_' . $root['id'] . '" data-commentid="' . $root['id'] . '" data-task-attachment-id="' . $root['file_id'] . '" class="tc-content task-comment' . (strtotime($root['dateadded']) >= strtotime('-16 hours') ? ' highlight-bg' : '') . '">';
                        $rootcause .= '<a data-task-corrective-href-id="' . $root['id'] . '" href="' . admin_url('tasks/view/' . $task->id) . '#corrective_' . $root['id'] . '" class="task-date-as-comment-id"><small><span class="text-has-action inline-block mbot5" data-toggle="tooltip" data-title="' . _dt($root['dateadded']) . '">' . time_ago($root['dateadded']) . '</span></small></a>';
                        if ($root['staffid'] != 0) {
                            $rootcause .= '<a href="' . admin_url('profile/' . $root['staffid']) . '" target="_blank">' . staff_profile_image($root['staffid'], array(
                        ...
                        if ($root['staffid'] == get_staff_user_id() || is_admin()) {
                            ...
                            $minus_1_hour = strtotime('-1 hours');
                            if (get_option('client_staff_add_edit_delete_task_comments_first_hour') == 0 || (get_option('client_staff_add_edit_delete_task_comments_first_hour') == 1 && $root_added >= $minus_1_hour) || is_admin()) {
                                $rootcause .= '<span class="pull-right"><a href="#" onclick="remove_task_preventive_action(' . $root['id'] . '); return false;"><i class="fa fa-times text-danger"></i></span></a>';
                                $rootcause .= '<span class="pull-right mright5"><a href="#" onclick="edit_task_preventive_action(' . $root['id'] . '); return false;"><i class="fa fa-pencil-square-o"></i></span></a>';
                            }
                        }
                        $rootcause .= '<div class="media-body">';
                        if ($root['staffid'] != 0) {
                        ...
                        $rootcause .= '<div data-edit-preventive="' . $root['id'] . '" class="hide edit-task-comment"><textarea rows="5" id="task_preventive_' . $root['id'] . '" class="ays-ignore form-control">' . str_replace('[task_attachment]', '', $root['content']) . '</textarea>
                  <div class="clearfix mtop20"></div>
                  <button type="button" class="btn btn-info pull-right" onclick="save_edited_preventive_action(' . $root['id'] . ',' . $task->id . ')">' . _l('submit') . '</button>
                  <button type="button" class="btn btn-default pull-right mright5" onclick="cancel_edit_preventive_action(' . $root['id'] . ')">' . _l('cancel') . '</button>
                  </div>';
                        if ($root['file_id'] != 0) {
                            $root['content'] = str_replace('[task_attachment]', '<div class="clearfix"></div>' . $attachments_dataAll['task-root_cause'][$root['file_id']], $root['content']);
                            // Replace lightbox to prevent loading the image twice
                            $root['content'] = str_replace('data-lightbox="task-attachment"', 'data-lightbox="task-attachment-comment"', $root['content']);
                        } else if (count($root['attachments']) > 0 && isset($preventive_attachments[$root['id']])) {
                            $root_attachments_html = '';
                            foreach ($preventive_attachments[$root['id']] as $root_attachment) {
                                $root_attachments_html .= trim($root_attachment);
                            }
                            $root['content'] = str_replace('[task_attachment]', '<div class="clearfix"></div>' . $root_attachments_html, $root['content']);
                            // Replace lightbox to prevent loading the image twice
                            ...
                            $root['content'] .='<div class="text-center download-all">
                   <hr class="hr-10" />
                   <a href="' . admin_url('tasks/download_files_all/3/' . $task->id . '/' . $root['id']) . '" class="bold">' . _l('download_all') . ' (.zip)
                   </a>
                   </div>';
                        }
                        $rootcause .= '<div class="comment-content mtop10">' . app_happy_text(check_for_links($root['content'])) . '</div>';
        ...
        <div class="col-md-4 task-single-col-right">
            <div class="pull-right mbot10 task-single-menu task-menu-options">
                <div class="content-menu hide">
                    <ul>
                        <?php if (has_permission('tasks', '', 'edit')) { ?>
                            <li>
                                <a href="#" onclick="edit_task(<?php echo $task->id; ?>);
                                            return false;">
                                ...
                                <ul>
                                    ...
                                    foreach ($task_single_mark_as_statuses as $status) {
                                        ...
                                        <?php if ($task->status != $status['id']) { ?>
                                            <li>
                                                <a href="#" onclick="task_mark_as(<?php echo $status['id']; ?>,<?php echo $task->id; ?>);
                                                                    return false;">
            ...
            <div class="task-info task-info-due-date task-single-inline-wrap<?php
            if (!$task->duedate && !has_permission('edit', '', 'tasks')) {
                echo ' hide';
            }
            ?>"<?php
            if (!$task->duedate) {
                                ...
                                <ul>
                                    <?php foreach (get_tasks_priorities() as $priority) { ?>
                                        <?php if ($task->priority != $priority['id']) { ?>
                                            <li>
                                                <a href="#" onclick="task_change_priority(<?php echo $priority['id']; ?>,<?php echo $task->id; ?>);
                                                                    return false;">
            ...
            <?php if ((has_permission('tasks', '', 'create') || has_permission('tasks', '', 'edit'))) { ?>
                <div class="task-info task-info-hourly-rate">
                    <h5><i class="fa task-info-icon fa-fw fa-lg pull-left fa-clock-o"></i>
                        <?php echo _l('task_hourly_rate'); ?>: <?php
                ...
                <div class="task-info task-info-billable">
                    <h5><i class="fa task-info-icon fa-fw fa-lg pull-left fa fa-credit-card"></i>
                        ...
                        <?php } ?>
                        <?php
                        if ($task->rel_type == 'project' && $task->project_data->billing_type == 1) {
                ...
                <?php
                if ($task->billable == 1 && $task->billed == 0 && ($task->rel_type != 'project' || ($task->rel_type == 'project' && $task->project_data->billing_type != 1))) {
                    ?>
            ...
            <div class="clearfix"></div>
            <?php
            if ($task->current_user_is_assigned) {
                foreach ($task->assignees as $assignee) {
            ...
            <a href="#" onclick="new_task_reminder(<?php echo $task->id; ?>);
                    return false;">
                                    ...
                                    <p class="bold">
                                        ...
                                        <?php if ($reminder['creator'] == get_staff_user_id() || is_admin()) { ?>
                                            <?php if ($reminder['isnotified'] == 0) { ?>
                                                <a href="#" class="  text-muted" onclick="edit_reminder(<?php echo $reminder['id']; ?>, this);
                                                                        return false;">
                    ...
                    <select data-width="100%" <?php if ($task->rel_type == 'project') { ?> data-live-search-placeholder="<?php echo _l('search_project_members'); ?>" <?php } ?> data-task-id="<?php echo $task->id; ?>" id="add_task_assignees" class="text-muted task-action-select selectpicker" name="select-assignees" data-live-search="true" title='<?php echo _l('task_single_assignees_select_title'); ?>' data-none-selected-text="<?php echo _l('dropdown_non_selected_tex'); ?>">
                        ...
                        foreach ($staff as $assignee) {
                            if (!in_array($assignee['staffid'], $task->assignees_ids)) {
                                if ($task->rel_type == 'project' && total_rows(db_prefix() . 'project_members', array('project_id' => $task->rel_id, 'staff_id' => $assignee['staffid'])) == 0) {
                                    continue;
                                }
