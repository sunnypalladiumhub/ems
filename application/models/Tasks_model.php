...
class Tasks_model extends App_Model {
    ...
    public function get($id, $where = []) {
        ...
        if ($task) {
            ...
            $task->root_cause = $this->get_task_new_details($id, 'task_root_cause', ['task-root_cause']);
            $task->corrective = $this->get_task_new_details($id, 'task_corrective', ['task-corrective']);
            $task->preventive = $this->get_task_new_details($id, 'task_preventive', ['task-preventive']);
            ...
            $attachment[] = $this->get_task_attachments($id);
            $attachment[] = $this->get_task_attachments_all($id, 'task_root_cause', ['task-root_cause']);
            $attachment[] = $this->get_task_attachments_all($id, 'task_corrective', ['task-corrective']);
            $attachment[] = $this->get_task_attachments_all($id, 'task_preventive', ['task-preventive']);
//            array_push($attachment,$this->get_task_attachments_all($id, 'task_root_cause', 'task-root_cause'));
//            $task->attachments = $this->get_task_attachments($id);
    ...
    /**
     * Add new task root cause
     * @param array $data root cause $_POST data
     * @return boolean
     */
    public function add_task_root_cause($data) {
        if (is_client_logged_in()) {
            $data['staffid'] = 0;
            $data['contact_id'] = get_contact_user_id();
        } else {
        ...
        $this->db->insert(db_prefix() . 'task_root_cause', [
            'taskid' => $data['taskid'],
            'content' => is_client_logged_in() ? _strip_tags($data['content']) : $data['content'],
            'staffid' => $data['staffid'],
            'contact_id' => $data['contact_id'],
    ...
    public function add_task_corrective_action($data) {
        if (is_client_logged_in()) {
            $data['staffid'] = 0;
            $data['contact_id'] = get_contact_user_id();
        } else {
        ...
        $this->db->insert(db_prefix() . 'task_corrective', [
            'taskid' => $data['taskid'],
            'content' => is_client_logged_in() ? _strip_tags($data['content']) : $data['content'],
            'staffid' => $data['staffid'],
            'contact_id' => $data['contact_id'],
    ...
    public function add_task_preventive_action($data) {
        if (is_client_logged_in()) {
            $data['staffid'] = 0;
            $data['contact_id'] = get_contact_user_id();
        } else {
        ...
        $this->db->insert(db_prefix() . 'task_preventive', [
            'taskid' => $data['taskid'],
            'content' => is_client_logged_in() ? _strip_tags($data['content']) : $data['content'],
            'staffid' => $data['staffid'],
            'contact_id' => $data['contact_id'],
    ...
    public function get_task_attachments_all($taskid, $table, $type, $where = []) {
        $this->db->select(implode(', ', prefixed_table_fields_array(db_prefix() . 'files')));
        $this->db->where(db_prefix() . 'files.rel_id', $taskid);
        $this->db->where_in(db_prefix() . 'files.rel_type', $type);

        if ((is_array($where) && count($where) > 0) || (is_string($where) && $where != '')) {
            $this->db->where($where);
        }

        if (in_array('task-root_cause', $type)) {
            $this->db->select(db_prefix() . 'task_root_cause.id as comment_file_id');
            $this->db->join(db_prefix() . 'task_root_cause', db_prefix() . 'task_root_cause.file_id = ' . db_prefix() . 'files.id', 'left');
        }
        if (in_array('task-corrective', $type)) {
            $this->db->select(db_prefix() . 'task_corrective.id as comment_file_id');
            $this->db->join(db_prefix() . 'task_corrective', db_prefix() . 'task_corrective.file_id = ' . db_prefix() . 'files.id', 'left');
        }
        if (in_array('task-preventive', $type)) {
            $this->db->select(db_prefix() . 'task_preventive.id as comment_file_id');
            $this->db->join(db_prefix() . 'task_preventive', db_prefix() . 'task_preventive.file_id = ' . db_prefix() . 'files.id', 'left');
        }
     ...
     * @return array
     */
    public function get_task_new_details($id, $table, $type) {
        $task_comments_order = hooks()->apply_filters('task_comments_order', 'DESC');

        $this->db->select('id,dateadded,content,' . db_prefix() . 'staff.firstname,' . db_prefix() . 'staff.lastname,' . db_prefix() . $table . '.staffid,' . db_prefix() . $table . '.contact_id as contact_id,file_id,CONCAT(firstname, " ", lastname) as staff_full_name');
        $this->db->from(db_prefix() . $table);
        $this->db->join(db_prefix() . 'staff', db_prefix() . 'staff.staffid = ' . db_prefix() . $table . '.staffid', 'left');
        $this->db->where('taskid', $id);
        $this->db->order_by('dateadded', $task_comments_order);
        ...
        if (count($ids) > 0) {
            $allAttachments = $this->get_task_attachments_all($id, $table, $type, 'task_comment_id IN (' . implode(',', $ids) . ')');
            foreach ($comments as $key => $comment) {
                foreach ($allAttachments as $attachment) {
                    if ($comment['id'] == $attachment['task_comment_id']) {
                        $comments[$key]['attachments'][] = $attachment;
    ...
    public function edit_root_cause($data) {
        // Check if user really creator
        $this->db->where('id', $data['id']);
        $comment = $this->db->get(db_prefix() . 'task_root_cause')->row();
        if ($comment->staffid == get_staff_user_id() || has_permission('tasks', '', 'edit') || $comment->contact_id == get_contact_user_id()) {
            $comment_added = strtotime($comment->dateadded);
            $minus_1_hour = strtotime('-1 hours');
            if (get_option('client_staff_add_edit_delete_task_comments_first_hour') == 0 || (get_option('client_staff_add_edit_delete_task_comments_first_hour') == 1 && $comment_added >= $minus_1_hour) || is_admin()) {
                ...
                $this->db->where('id', $data['id']);
                $this->db->update(db_prefix() . 'task_root_cause', [
                    'content' => $data['content'],
                ]);
                if ($this->db->affected_rows() > 0) {
    ...
    public function edit_corrective_action($data) {
        // Check if user really creator
        $this->db->where('id', $data['id']);
        $comment = $this->db->get(db_prefix() . 'task_corrective')->row();
        if ($comment->staffid == get_staff_user_id() || has_permission('tasks', '', 'edit') || $comment->contact_id == get_contact_user_id()) {
            $comment_added = strtotime($comment->dateadded);
            $minus_1_hour = strtotime('-1 hours');
            if (get_option('client_staff_add_edit_delete_task_comments_first_hour') == 0 || (get_option('client_staff_add_edit_delete_task_comments_first_hour') == 1 && $comment_added >= $minus_1_hour) || is_admin()) {
                ...
                $this->db->where('id', $data['id']);
                $this->db->update(db_prefix() . 'task_corrective', [
                    'content' => $data['content'],
                ]);
                if ($this->db->affected_rows() > 0) {
    ...
    public function edit_preventive_action($data) {
        // Check if user really creator
        $this->db->where('id', $data['id']);
        $comment = $this->db->get(db_prefix() . 'task_preventive')->row();
        if ($comment->staffid == get_staff_user_id() || has_permission('tasks', '', 'edit') || $comment->contact_id == get_contact_user_id()) {
            $comment_added = strtotime($comment->dateadded);
            $minus_1_hour = strtotime('-1 hours');
            if (get_option('client_staff_add_edit_delete_task_comments_first_hour') == 0 || (get_option('client_staff_add_edit_delete_task_comments_first_hour') == 1 && $comment_added >= $minus_1_hour) || is_admin()) {
                ...
                $this->db->where('id', $data['id']);
                $this->db->update(db_prefix() . 'task_preventive', [
                    'content' => $data['content'],
                ]);
                if ($this->db->affected_rows() > 0) {
    ...
    public function remove_task_new_detail($id, $table, $filetype, $force = false) {
        // Check if user really creator
        $this->db->where('id', $id);
        $comment = $this->db->get(db_prefix() . $table)->row();
...
//                    $commentAttachments = $this->get_task_attachments($comment->taskid, 'task_comment_id=' . $id);
                    $commentAttachments = $this->get_task_attachments_all($comment->taskid, $table, $filetype);
                    foreach ($commentAttachments as $attachment) {
                        $this->remove_task_attachment($attachment['id']);
                    }
...
//            }
    ...
    public function timer_tracking($task_id = '', $timer_id = '', $note = '', $adminStop = false) {
        ...
        if ($newTimer) {
            ...
            if ($task_id != '0' && get_option('timer_started_change_status_in_progress') == '1' && total_rows(db_prefix() . 'tasks', ['id' => $task_id, 'status' => 1]) > 0) {
    ...
    private function should_staff_receive_notification($staffid, $taskid) {
        ...
        return ($this->is_task_assignee($staffid, $taskid) || $this->is_task_follower($staffid, $taskid) || $this->is_task_creator($staffid, $taskid) || $this->staff_has_commented_on_task($staffid, $taskid));
    }
